                             ...
                             char *a_base_out, char *a_quote_out, uint8_t *a_side_out)
{
    if (strcmp(a_sell_token, a_buy_token) < 0) {
        ...
        dap_strncpy(a_base_out, a_sell_token, DAP_CHAIN_TICKER_SIZE_MAX);
        dap_strncpy(a_quote_out, a_buy_token, DAP_CHAIN_TICKER_SIZE_MAX);
        if (a_side_out) *a_side_out = 0; // ASK
    } else {
        // sell >= buy: BASE=buy, QUOTE=sell → BID
        dap_strncpy(a_base_out, a_buy_token, DAP_CHAIN_TICKER_SIZE_MAX);
        dap_strncpy(a_quote_out, a_sell_token, DAP_CHAIN_TICKER_SIZE_MAX);
...
PyObject *wrapping_dap_chain_net_srv_dex_get_orders(PyObject *self, PyObject *args, PyObject *kwargs)
{
    ...
    if (pair_str && pair_str[0]) {
        ...
        *l_slash++ = '\0';
        ...
        if (strcmp(l_pair_storage, l_slash) < 0) {
            dap_strncpy(l_ticker_base, l_pair_storage, DAP_CHAIN_TICKER_SIZE_MAX);
            dap_strncpy(l_ticker_quote, l_slash, DAP_CHAIN_TICKER_SIZE_MAX);
        } else {
            dap_strncpy(l_ticker_base, l_slash, DAP_CHAIN_TICKER_SIZE_MAX);
            dap_strncpy(l_ticker_quote, l_pair_storage, DAP_CHAIN_TICKER_SIZE_MAX);
        }
        l_has_pair = true;
    }
    ...
    for (dap_chain_datum_tx_t *l_tx = dap_ledger_datum_iter_get_first(it); 
        ...
        // Tokens
        dap_strncpy(l_order->token_sell, l_sell_tok, DAP_CHAIN_TICKER_SIZE_MAX);
        dap_strncpy(l_order->token_buy, l_out_cond->subtype.srv_dex.buy_token, DAP_CHAIN_TICKER_SIZE_MAX);
