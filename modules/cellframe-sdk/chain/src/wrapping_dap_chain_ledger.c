...
static bool s_python_obj_notificator(UNUSED_ARG dap_proc_thread_t *a_thread, void *a_arg)
{
    ...
    PyObject *l_notify_arg = l_notificator->argv ? l_notificator->argv : ( Py_INCREF(Py_None), Py_None );
...
static PyObject *s_bridged_tx_notify_add(PyObject *self, PyObject *args)
{
    ...
    if (!l_notificator) {
        log_it(L_ERROR, "Memory allocation error in dap_chain_ledger_bridged_tx_notify_add_py");
        return NULL;
    }
