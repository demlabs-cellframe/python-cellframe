...
PyObject *wrapping_dap_global_db_gr_set(PyObject *self, PyObject *args){
    ...
    int ret = dap_global_db_set(l_group, l_key, l_bytes, l_bytes_size, l_is_pinned, NULL, 0);
...
PyObject *wrapping_dap_global_db_gr_del(PyObject *self, PyObject *args){
    ...
    int ret = dap_global_db_del(l_group, l_key, NULL, 0);
    if (ret == DAP_GLOBAL_DB_RC_SUCCESS)
        Py_RETURN_TRUE;
    else
        Py_RETURN_FALSE;
...
PyObject *wrapping_dap_global_db_gr_pin(PyObject *self, PyObject *args){
    ...
    int ret = dap_global_db_pin(l_group, l_key, NULL, 0);
    if (ret == DAP_GLOBAL_DB_RC_SUCCESS)
        Py_RETURN_TRUE;
    else
        Py_RETURN_FALSE;
...
PyObject *wrapping_dap_global_db_gr_unpin(PyObject *self, PyObject *args){
    ...
    int ret = dap_global_db_unpin(l_group, l_key, NULL, 0);
    if (ret == DAP_GLOBAL_DB_RC_SUCCESS)
        Py_RETURN_TRUE;
    else
        Py_RETURN_FALSE;
...
void pvt_wrapping_dap_global_db_add_sync_extra_group_func_callback(dap_global_db_context_t *a_context, dap_store_obj_t *a_obj, void *a_arg)
{
    ...
    if (a_obj->value == NULL || a_obj->value_len == 0){
        l_obj_value = Py_None;
    } else {
        l_obj_value = PyBytes_FromStringAndSize((const char *)a_obj->value, (Py_ssize_t)a_obj->value_len);
    }
