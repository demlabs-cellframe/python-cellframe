...
dap_chain_addr_t *dap_chain_addr_from_pyobj(PyObject *obj, size_t *o_count) {
    ...
    if (PyList_Check(obj)) {
        ...
        *o_count = PyList_Size(obj);
        ...
        while ((item = PyIter_Next(iter)) != NULL) {
            if (!PyDapChainAddrObject_Check((PyDapChainAddrObject*)item)) {
                PyErr_SetString(PyExc_TypeError, "List must contain only DapChainAddr objects");
                DAP_DELETE(res);
                return NULL;
            }
            *(res + l_pos_valid++) = *((PyDapChainAddrObject*)item)->addr;
        ...
        if (!PyDapChainAddrObject_Check((PyDapChainAddrObject*)obj)) {
