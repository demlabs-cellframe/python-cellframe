...
if TYPE_CHECKING:
    ...
    # Math available through python_dap

from .datums import CfDatum
from dap.crypto import DapSign

class CfAtom:
    """Base class for chain atoms (blocks/events)"""
    
    @classmethod
    def get_consensus_type(cls) -> str:
        """Get consensus type for this atom type"""
        raise NotImplementedError("Subclasses must implement get_consensus_type")
    
    @classmethod
    def is_supported_by_chain(cls, chain_type: str) -> bool:
        """Check if this atom type is supported by chain type"""
        return chain_type == cls.get_consensus_type()


class CfBlock(CfAtom):
    """The CellFrame block.
    ...
    Attributes:
        ...
        signs (list[DapSign]): A list of CfSign objects representing the signatures of the block.
    ...
    def __init__(self, atom: ChainAtomPtr, chain: CFChain, size: int = 0):
        ...
        self.signs = [DapSign(sign, self.chain.net) for sign in block.signs]
    ...
    @classmethod
    def get_consensus_type(cls) -> str:
        """Get consensus type for blocks"""
        return "esbocs"


class CfEvent(CfAtom):
    """The CellFrame event.
...
class CfStakePosDelegate:
    ...
    def get_validator_info(self) -> CFValidatorInfo:
        ...
        try:
            ...
            # Use python_dap logging or standard logging
            import logging
            logger = logging.getLogger(__name__)
            logger.error(f"Error retrieving validator info for network {self._net.name}: {traceback.format_exc()}")
            raise e
